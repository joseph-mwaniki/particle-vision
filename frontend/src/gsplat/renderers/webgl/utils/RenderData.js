...
class RenderData {
    ...
    _transformsHeight;
    _transformIndices;
    _transformIndicesWidth;
    _transformIndicesHeight;
    _colorTransforms;
    _colorTransformsWidth;
    _colorTransformsHeight;
    _colorTransformIndices;
    _colorTransformIndicesWidth;
    _colorTransformIndicesHeight;
    _positions;
    _rotations;
    _scales;
    _vertexCount;
    ...
    constructor(scene) {
        ...
        this._colorTransformIndices = new Uint32Array(this._colorTransformIndicesWidth * this._colorTransformIndicesHeight);
        ...
        const updateTransform = (splat) => {
            const splatIndex = this._splatIndices.get(splat);
        ...
        const updateColorTransforms = () => {
            ...
            const colorTransformsMap = [new Matrix4()];
            this._colorTransformIndices.fill(0);
            let i = 1;
            for (const splat of this._splatIndices.keys()) {
                const offset = this._offsets.get(splat);
                ...
                for (const index of splat.colorTransformsMap.keys()) {
                    const colorTransformIndex = splat.colorTransformsMap.get(index);
        ...
        this._worker.onmessage = (e) => {
            if (e.data.response) {
                ...
                const splat = lookup.get(response.offset);
                ...
                splat.data.reattach(response.positions, response.rotations, response.scales, response.colors, response.selection);
        ...
        const buildImmediate = (splat) => {
            ...
            wasmModule._pack(splat.selected, splat.data.vertexCount, positionsPtr, rotationsPtr, scalesPtr, colorsPtr, selectionPtr, dataPtr, worldPositionsPtr, worldRotationsPtr, worldScalesPtr);
            const outData = new Uint32Array(wasmModule.HEAPU32.buffer, dataPtr, splat.data.vertexCount * 8);
            const worldPositions = new Float32Array(wasmModule.HEAPF32.buffer, worldPositionsPtr, splat.data.vertexCount * 3);
            const worldRotations = new Float32Array(wasmModule.HEAPF32.buffer, worldRotationsPtr, splat.data.vertexCount * 4);
        ...
        const build = (splat) => {
            ...
            if (!splat.data.changed || splat.data.detached)
                return;
            const serializedSplat = {
                ...
                offset: this._offsets.get(splat),
            };
        ...
        this.getLocalIndex = (splat, index) => {
            const offset = this._offsets.get(splat);
            return index - offset;
        };
        this.markDirty = (splat) => {
