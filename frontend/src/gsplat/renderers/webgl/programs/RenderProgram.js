...
class RenderProgram extends ShaderProgram {
    _outlineThickness = 10.0;
    _outlineColor = new Color32(255, 165, 0, 255);
    _renderData = null;
    _depthIndex = new Uint32Array();
    _splatTexture = null;
    _worker = null;
    _initialize;
    _resize;
    ...
    constructor(renderer, passes) {
        ...
        let u_texture;
        let u_transforms;
        let u_transformIndices;
        let u_colorTransforms;
        let u_colorTransformIndices;
        let u_outlineThickness;
        let u_outlineColor;
        let positionAttribute;
        let indexAttribute;
        let transformsTexture;
        let transformIndicesTexture;
        let colorTransformsTexture;
        let colorTransformIndicesTexture;
        let vertexBuffer;
        let indexBuffer;
        this._resize = () => {
            if (!this._camera)
        ...
        this._initialize = () => {
            ...
            u_outlineThickness = gl.getUniformLocation(this.program, "outlineThickness");
            gl.uniform1f(u_outlineThickness, this.outlineThickness);
            u_outlineColor = gl.getUniformLocation(this.program, "outlineColor");
            gl.uniform4fv(u_outlineColor, new Float32Array(this.outlineColor.flatNorm()));
            this._splatTexture = gl.createTexture();
            u_texture = gl.getUniformLocation(this.program, "u_texture");
            gl.uniform1i(u_texture, 0);
            transformsTexture = gl.createTexture();
            u_transforms = gl.getUniformLocation(this.program, "u_transforms");
            gl.uniform1i(u_transforms, 1);
            transformIndicesTexture = gl.createTexture();
            u_transformIndices = gl.getUniformLocation(this.program, "u_transformIndices");
            gl.uniform1i(u_transformIndices, 2);
            colorTransformsTexture = gl.createTexture();
            u_colorTransforms = gl.getUniformLocation(this.program, "u_colorTransforms");
            gl.uniform1i(u_colorTransforms, 3);
            colorTransformIndicesTexture = gl.createTexture();
            u_colorTransformIndices = gl.getUniformLocation(this.program, "u_colorTransformIndices");
        ...
        const handleObjectChanged = (event) => {
            const e = event;
        ...
        this._render = () => {
            ...
            if (this.renderData.dataChanged ||
                ...
                if (this.renderData.dataChanged) {
                    ...
                    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA32UI, this.renderData.width, this.renderData.height, 0, gl.RGBA_INTEGER, gl.UNSIGNED_INT, this.renderData.data);
                }
                if (this.renderData.transformsChanged) {
                    ...
                    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
                    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
                    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA32F, this.renderData.transformsWidth, this.renderData.transformsHeight, 0, gl.RGBA, gl.FLOAT, this.renderData.transforms);
                    ...
                    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
                    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
                    gl.texImage2D(gl.TEXTURE_2D, 0, gl.R32UI, this.renderData.transformIndicesWidth, this.renderData.transformIndicesHeight, 0, gl.RED_INTEGER, gl.UNSIGNED_INT, this.renderData.transformIndices);
                }
                if (this.renderData.colorTransformsChanged) {
                    ...
                    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
                    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
                    gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA32F, this.renderData.colorTransformsWidth, this.renderData.colorTransformsHeight, 0, gl.RGBA, gl.FLOAT, this.renderData.colorTransforms);
                    ...
                    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MIN_FILTER, gl.NEAREST);
                    gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.NEAREST);
                    gl.texImage2D(gl.TEXTURE_2D, 0, gl.R32UI, this.renderData.colorTransformIndicesWidth, this.renderData.colorTransformIndicesHeight, 0, gl.RED_INTEGER, gl.UNSIGNED_INT, this.renderData.colorTransformIndices);
                }
