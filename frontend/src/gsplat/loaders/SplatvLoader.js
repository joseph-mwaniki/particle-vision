...
class SplatvLoader {
    static async LoadAsync(url, scene, camera, onProgress, useCache = false) {
    ...
    static async LoadFromFileAsync(file, scene, camera, onProgress) {
        const reader = new FileReader();
        let splatv = null;
        reader.onload = (e) => {
            splatv = this._ParseSplatvBuffer(e.target.result, scene, camera);
        };
        reader.onprogress = (e) => {
    ...
    static _ParseSplatvBuffer(inputBuffer, scene, camera) {
        let result = null;
        const handleChunk = (chunk, buffer, chunks) => {
            if (chunk.type === "magic") {
                ...
                if (camera && cameras && cameras.length) {
                    ...
                    const position = new Vector3(cameraData.position[0], cameraData.position[1], cameraData.position[2]);
                    const rotation = Quaternion.FromMatrix3(new Matrix3(cameraData.rotation[0][0], cameraData.rotation[0][1], cameraData.rotation[0][2], cameraData.rotation[1][0], cameraData.rotation[1][1], cameraData.rotation[1][2], cameraData.rotation[2][0], cameraData.rotation[2][1], cameraData.rotation[2][2]));
