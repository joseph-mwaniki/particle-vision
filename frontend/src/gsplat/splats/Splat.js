...
class Splat extends Object3D {
    ...
    _selected = false;
    _colorTransforms = [];
    _colorTransformsMap = new Map();
    _bounds;
    recalculateBounds;
    constructor(splat = undefined) {
        super();
        this._data = splat || new SplatData();
        this._bounds = new Box3(new Vector3(Infinity, Infinity, Infinity), new Vector3(-Infinity, -Infinity, -Infinity));
        this.recalculateBounds = () => {
            this._bounds = new Box3(new Vector3(Infinity, Infinity, Infinity), new Vector3(-Infinity, -Infinity, -Infinity));
            for (let i = 0; i < this._data.vertexCount; i++) {
                this._bounds.expand(new Vector3(this._data.positions[3 * i], this._data.positions[3 * i + 1], this._data.positions[3 * i + 2]));
            }
        };
        this.applyPosition = () => {
