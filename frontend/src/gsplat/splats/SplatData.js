...
class SplatData {
    ...
    serialize;
    reattach;
    constructor(vertexCount = 0, positions = null, rotations = null, scales = null, colors = null) {
        ...
        this.rotate = (rotation) => {
            ...
            for (let i = 0; i < this.vertexCount; i++) {
                ...
                const currentRotation = new Quaternion(this.rotations[4 * i + 1], this.rotations[4 * i + 2], this.rotations[4 * i + 3], this.rotations[4 * i + 0]);
        ...
        this.reattach = (positions, rotations, scales, colors, selection) => {
            console.assert(positions.byteLength === this.vertexCount * 3 * 4, `Expected ${this.vertexCount * 3 * 4} bytes, got ${positions.byteLength} bytes`);
    ...
    clone() {
        return new SplatData(this.vertexCount, new Float32Array(this.positions), new Float32Array(this.rotations), new Float32Array(this.scales), new Uint8Array(this.colors));
    }
}
export { SplatData };
